component extends="Controller" hint="Controller for registered members section" {
    ...
    public any function settings() hint="Current user settings" {

        var local = {};

        _view(pageTitle = "Personal Settings", buttonLabel = "Save Settings", renderFlash = false);
        _view(headLink = linkTo(text="Personal Profile", action="profile"));

        warnings = [];


        // read the setting values joined with tokens

        settings = model("settingvalue").findAll(
            where="accesslevel = #get('accessLevelMember')# AND userid = #getUserAttr('id')#",
            order="description asc",
            include="settingtoken"
        );


        // sync values with tokens

        local.settingtokens = model("settingtoken").findAll(
            where="accesslevel = #get('accessLevelMember')#",
            order="description asc"
        );


        if (settings.recordCount LT local.settingtokens.recordCount) {

            syncSettingValues(local.settingtokens, getUserAttr('accountid'), getUserAttr('id'));
        ...
        // custom column for errors status
        QueryAddColumn(settings, "invalid");
    ...
    public any function settingsUpdate() hint="Save current user settings" {
        ...
        param name="params.settings" default={};


        // search each setting and validate the values
        warnings = checkSettingValues(settings, params.settings);


        // persist the values or display warnings

        if (ArrayLen(warnings) EQ 0) {

            saveSettingValues(settings, params.settings);

            flashInsert(success="Settings were updated successfully");
        ...
        if (StructKeyExists(params, "email")) {
            ...
            try {
                ...
                local.maildata = {
                    ...
                    link : linkTo(route="passwordConfirm", stamp=crypt(local.user.id & "-sa1t", true), onlyPath=false)
                }
    ...
    public any function signup() hint="Member registration pre- and post-processing steps" {
        ...
        if (StructKeyExists(params, "accountname")) {
            ...
            try {
                ...
                param name="local.account.id" default="10";

                params.accountplan = {accountid : local.account.id, planid : 2, discountid : 1};

                params.user = {
                    "accountid" : local.account.id,
                    "accesslevel" : get("accessLevelManager"),
                ...
                // buld the payment form link and relocate to it

                local.paymentUrl = Replace(get("hostedPaymentURL"), "{plan_code}", "beta");
                local.paymentUrl = Replace(local.paymentUrl, "{account_code}", local.account.code);
                local.paymentUrl = Replace(local.paymentUrl, "{email}", local.user.email);

                location(local.paymentUrl, false);
